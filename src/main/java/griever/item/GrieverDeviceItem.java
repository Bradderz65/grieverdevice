...
public class GrieverDeviceItem extends Item {
    ...
    private static final int MIN_BEEP_INTERVAL = 2; // Fastest beeping rate (for very close proximity)
    ...
    private int calculateBeepInterval(double distance) {
        ...
        // Calculate the interval using a more precise non-linear curve
        // Avoid expensive Math.pow operation by using a custom approximation
        double distanceRatio = clampedDistance * INVERSE_MAX_DISTANCE;
        double adjustedRatio = distanceRatio * Math.sqrt(distanceRatio); // Equivalent to pow(1.5) but faster
        
        // Calculate the final interval
        return Math.max(MIN_BEEP_INTERVAL, (int)(MIN_BEEP_INTERVAL + (adjustedRatio * INTERVAL_RANGE)));
    }
    ...
    private void processDeviceTick(ItemStack stack, World world, PlayerEntity player) {
        ...
        // Use a smaller threshold to detect significant distance changes
        double distanceChangeThresholdSquared = 0.25; // 0.5 squared
        
        int beepInterval = calculateBeepInterval(currentDistance);
        
        if (currentDistanceSquared > lastDistanceSquared + distanceChangeThresholdSquared) {
            beepInterval = Math.min(MAX_BEEP_INTERVAL, beepInterval * 2);
        }
        ...
        if (beepTimer >= beepInterval) {
            long lastBeepTime = playerLastBeepTimes.getLong(playerUuid);
            if (!world.isClient && (currentTime - lastBeepTime >= 2)) {
